...
El proyecto incluye un actualizador ejecutable y una integración con la calculadora. Consulta un catálogo acotado de productos; **no rastrea todo el supermercado**. Python 3.11 o superior es suficiente, sin dependencias externas.
...
La descarga de precios ocurre fuera del navegador, evitando consultas a las tiendas desde cada visitante. No necesita un servidor web de Python ni un endpoint público que acepte URLs. El script hace solicitudes secuenciales, con intervalo mínimo, límite de tamaño y tiempo máximo de conexión.
...
python3 pricing/sync_prices.py --ingredient panCompleto
```
...
En el administrador de tareas cron, configura una ejecución cada seis horas. Cambia las rutas de este ejemplo por las reales de tu hosting; verifica también la ubicación de Python:

```cron
23 */6 * * * /usr/bin/python3 /home/TU_USUARIO/la-junta/pricing/sync_prices.py --config /home/TU_USUARIO/la-junta/pricing/sources.json --output /home/TU_USUARIO/public_html/prices.json >> /home/TU_USUARIO/la-junta/precios.log 2>&1
```
...
Si tu hosting no permite Python o cron, usa la opción B.
...
Las pruebas no se conectan a tiendas. Validan la secuencia de fuentes, conservación de fechas, rechazos por producto o moneda incorrectos, bloqueos, extracción estructurada y selectores, además del presupuesto con formatos reales. La comprobación de una tienda en producción es un paso separado de instalación.
